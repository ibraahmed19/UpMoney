...
// create new user
func NewUser(db *gorm.DB, user common.User) (common.User, error) {
	return user, db.Create(&user).Error
}

// get all users
func GetUsers(db *gorm.DB) (users []common.User, err error) {
	return users, db.Find(&users).Error
}

// search users
func SearchUsers(db *gorm.DB, user common.User) (users []common.User, err error) {
	return users, db.Where(&user).Find(&users).Error
}

// update user
func UpdateUser(db *gorm.DB, user common.User) error {
	return db.Where("id=?", user.ID).Updates(&user).Error
}

// delete user
func DeleteUser(db *gorm.DB, user_id uint) error {
	return db.Where("id=?", user_id).Delete(&common.User{}).Error
}

// get user by email
func GetUserByEmail(db *gorm.DB, email string) (user common.User, err error) {
	return user, db.First(&user, "email=?", email).Error
}

// get user by id
func GetUserByID(db *gorm.DB, id uint) (user common.User, err error) {
