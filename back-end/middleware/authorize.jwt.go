...
func GenerateToken(id uint, roles string) string {
...
func ExtractTokenValues(ctx *gin.Context) Session {
	...
	if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
		...
		// session.BankID = uint(claims["bank_id"].(float64))
		// session.InsuranceID = uint(claims["insurance_id"].(float64))
		return session
	}
	return Session{}
}
		...
		if token, err := validateToken(tokenString); err != nil {
			...
			if claims, ok := token.Claims.(jwt.MapClaims); !ok {
				...
				if token.Valid {
					...
					ctx.Set("bank_id", claims["bank_id"])
					ctx.Set("insurance_id", claims["insurance_id"])
